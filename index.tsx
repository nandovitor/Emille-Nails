...
function usePersistentState<T>(key: string, initialState: T): [T, React.Dispatch<React.SetStateAction<T>>] {
    ...
    useEffect(() => {
        try {
            ...
            if (key === 'bookingState' && (state as any).selectedServices instanceof Map) {
                valueToStore = {
                    ...state,
                    selectedServices: Array.from((state as any).selectedServices.entries())
                };
            }
...
const App: React.FC = () => {
  const [bookingState, setBookingState] = usePersistentState('bookingState', {
    ...
    selectedPedicureOption: null as string | null,
    selectedDate: '',
    selectedTime: '',
    userInfo: { name: '', phone: '' } as UserInfo,
  });
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'SERVICES': return <ServicesPage {...pageProps} onServiceToggle={handleServiceToggle} onPedicureOptionChange={handlePedicureOptionChange} onNext={() => updateState({ currentPage: 'DATETIME' })} onBack={() => updateState({ currentPage: 'HOME' })} />;
      case 'DATETIME': return <DateTimePage {...pageProps} onNext={() => updateState({ currentPage: 'USER_INFO' })} onBack={() => updateState({ currentPage: 'SERVICES' })} />;
      case 'USER_INFO': return <UserInfoPage {...pageProps} onNext={() => updateState({ currentPage: 'CONFIRM' })} onBack={() => updateState({ currentPage: 'DATETIME' })} />;
      case 'CONFIRM': return <ConfirmationPage {...pageProps} totalCost={totalCost} onConfirm={() => updateState({ currentPage: 'SUCCESS' })} onBack={() => updateState({ currentPage: 'USER_INFO' })} />;
...
const HomePage: React.FC<{ onNext: () => void; onModalOpen: (type: ModalType) => void; }> = ({ onNext, onModalOpen }) => (
  <div className="page">
...
const ServicesPage: React.FC<{
  ...
  onNext: () => void;
  onBack: () => void;
}> = ({ bookingState, onServiceToggle, onPedicureOptionChange, onNext, onBack }) => (
  <div className="page">
...
const DateTimePage: React.FC<{
  ...
  onNext: () => void;
  onBack: () => void;
}> = ({ bookingState, updateState, onNext, onBack }) => {
...
const ConfirmationPage: React.FC<{
  ...
  onConfirm: () => void;
  onBack: () => void;
}> = ({ bookingState, totalCost, onConfirm, onBack }) => {
